...
def summarize_video(
    ...
    """
    logger.info("Summarizing file_id=%s focus=%r", file_id, focus)
...
def detect_niche_topic(summary: str) -> tuple[str, str]:
    ...
    try:
        ...
        logger.warning("detect_niche_topic JSON parse failed: %s | raw=%r", e, raw)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python -m src.summarize.summarize <file_id>")
        sys.exit(1)
