...
def ingest_video(
    ...
    logger.info("Start ingest file_id=%s video=%s chunk_sec=%s", file_id, video_path, chunk_sec)
    ...
    logger.info("Inserted %d chunks for file_id=%s", n_chunks, file_id)
    ...
    from src.summarize.summarize import summarize_video, detect_niche_topic
    ...
    logger.info("Summary stored file_id=%s niche=%r topic=%r", file_id, niche, topic)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m src.ingest.ingest <video.mp4> [chunk_sec] [--prompt 'text']")
        sys.exit(1)
